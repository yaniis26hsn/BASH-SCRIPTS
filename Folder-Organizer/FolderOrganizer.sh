...
# first filtring all the files from folders 
 ls > allContent 
   while IFS= read -r line ; do   # IFS is necessary to take the whole line (each line is a separtor )
        if [ -f "$line" ];then
         echo "$line" >> FilesOnly  # we will store only files now
        fi 
        done < allContent  

# then getting extensions 

rev FilesOnly > reversedFile #rev cmd will make the same file but reversed (each line will be reversed from the right to the left )
...
#  then : replacing empty extensions (deleted after step 2) by the word "other"
...
#  since we can't read from two files in one time (by a simple way) we concatenate them to a new one and read from it as usual  
paste -d / FilesOnly fixedExtensions > FilesWithTheirExtensions # i choosed / as a separator cz you can't use it in the name of any file 
...
# next : moving files to their specific folder 
...
# finaly: deleting those temprary files 
